...
class _RecentLogsWidgetState extends State<RecentLogsWidget> {
  ...
  void initState() {
    ...
    Hive.box<OvernightLog>(overnightLogBoxName).listenable().addListener(
        _loadRecentLogs);
  }

  @override
  void dispose() {
    Hive.box<MealLog>(mealLogBoxName).listenable().removeListener(
        _loadRecentLogs);
    Hive.box<OvernightLog>(overnightLogBoxName).listenable().removeListener(
        _loadRecentLogs);
    super.dispose();
  }
  ...
  Future<void> _loadRecentLogs() async {
    ...
    try {
      _sortedRecentLogs =
      await _logRepository.getRecentLogs(const Duration(hours: 24));
    } catch (e) {
              ...
              if (type == 'meal' && log is MealLog) ...[
                ...
                if (type == 'overnight' && log is OvernightLog) ...[
                  Text('Hora de dormir: ${timeFormat.format(log.bedTime)}',
                      style: bottomSheetTheme.textTheme.bodyMedium),
                  Text('Glucosa antes de dormir: ${log
                      .beforeSleepBloodSugar} mg/dL',
                      style: bottomSheetTheme.textTheme.bodyMedium),
  ...
  Widget build(BuildContext context) {
    ...
    return LoadingOrEmptyStateWidget(
      ...
      // Usar el estado local _isLoading
      loadingText: "Cargando últimos registros...",
      // Este widget no maneja errores explícitamente en su estado actual,
      // pero si _loadRecentLogs() pudiera fallar y establecer un estado de error,
      // podrías pasarlo aquí.
      // hasError: _hasErrorState, // Si tuvieras _hasErrorState
      // errorMessage: _errorMessageState, // Si tuvieras _errorMessageState
      // onRetry: _loadRecentLogs, // Si tuvieras una forma de reintentar

      isEmpty: !_isLoading && _sortedRecentLogs.isEmpty,
      // Solo considerar vacío si no está cargando
      ...
      childIfData: SingleChildScrollView( // Esto es childIfData
        ...
        child: Row(
          children: _sortedRecentLogs.map((logData) {
            ...
            IconData iconData = type == 'meal' ? Icons.fastfood_rounded : Icons
                .bedtime_rounded;
            Color iconBackgroundColor = type == 'meal' ? theme.colorScheme
                .primaryContainer : theme.colorScheme.secondaryContainer;
            Color onIconBackgroundColor = type == 'meal' ? theme.colorScheme
                ...
                children: [
                  Container(
                    ...
                    child: IconButton(
                      ...
                      tooltip: '${type == 'meal'
                          ? 'Comida'
                          : 'Noche'} - ${DateFormat.Hm().format(time)}',
